...
- `GET /api/projects/{pid}/golden-dataset` — 获取项目下所有黄金记录
- `POST /api/projects/{pid}/golden-dataset` — 新增一条黄金记录
- `PUT /api/golden-dataset/{id}` — 更新一条黄金记录
- `DELETE /api/golden-dataset/{id}` — 删除一条黄金记录

#### Scenario: 获取黄金数据集列表
- **WHEN** 请求 `GET /api/projects/{pid}/golden-dataset`
- **THEN** 返回该项目下所有黄金记录，按 created_at DESC 排序

#### Scenario: 新增黄金记录
- **WHEN** 请求 `POST /api/projects/{pid}/golden-dataset`，body 包含 query、ground_truth_chunks、reference_answer
- **THEN** 创建记录并返回完整对象（含 id、created_at）

#### Scenario: 更新黄金记录
- **WHEN** 请求 `PUT /api/golden-dataset/{id}`，body 包含 query、ground_truth_chunks、reference_answer
- **THEN** 更新记录并返回更新后的对象

#### Scenario: 删除黄金记录
- **WHEN** 请求 `DELETE /api/golden-dataset/{id}`
- **THEN** 删除该记录，返回成功确认

#### Scenario: 删除不存在的记录
- **WHEN** 请求 `DELETE /api/golden-dataset/{id}` 且 id 不存在
- **THEN** 返回 404 错误
...
系统 SHALL 提供黄金数据集管理页面，路径为 `/golden-dataset`，依赖 activeProjectStore 获取当前项目。
...
侧边栏 SHALL 新增"黄金数据集"菜单项，图标使用 TrophyOutlined，路径为 `/golden-dataset`。

#### Scenario: 菜单导航
- **WHEN** 用户点击侧边栏"黄金数据集"
- **THEN** 导航到 `/golden-dataset` 页面
